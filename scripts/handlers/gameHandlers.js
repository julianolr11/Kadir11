...
function registerGameHandlers(deps) {
  ...
  ipcMain.on('battle-pet', async () => {
    ...
    if (!pet) {
      BrowserWindow.getAllWindows().forEach((w) => {
        if (w.webContents)
          w.webContents.send('show-battle-error', 'Nenhum pet selecionado para batalhar!');
      });
      return;
    }
    if (pet.energy < 5) {
      BrowserWindow.getAllWindows().forEach((w) => {
        if (w.webContents)
          w.webContents.send(
            'show-battle-error',
            'Descanse um pouco até sua próxima batalha! Energia insuficiente.'
          );
      });
      return;
    }
    ...
    if (pet.currentHealth < minHealth) {
      BrowserWindow.getAllWindows().forEach((w) => {
        if (w.webContents)
          w.webContents.send(
            'show-battle-error',
            'Seu pet precisa de pelo menos 30% de vida para batalhar.'
          );
      });
      return;
    }
    ...
    applyBattleProgress(
      pet,
      { baseXp: 10 },
      { calculateXpGain, getRequiredXpForNextLevel, increaseAttributesOnLevelUp }
    );
    try {
  ...
  ipcMain.on('journey-complete', async () => {
    ...
    applyJourneyRewards(
      pet,
      { eggId, coins: 50, kadirPoints: 100 },
      { getItems, setItems, getCoins, setCoins }
    );
    try {
  ...
  ipcMain.on('open-journey-mode-window', () => {
    ...
    if (pet && win) {
      ...
      // Se a janela já estava aberta e pronta, enviar pet-data imediatamente
  ...
  ipcMain.on('open-train-menu-window', () => {
    if (getCurrentPet()) createTrainMenuWindow();
  });
  ipcMain.on('open-train-attributes-window', () => {
    if (getCurrentPet()) createTrainAttributesWindow();
  });
  ...
  ipcMain.on('open-journey-scene-window', async (event, data) => {
    ...
    let enemyName;
    let enemyElement;
    // Se for boss final, usar Armored Griffin dos assets Boss
    if (data && data.boss === true) {
      // Caminhos POSIX para compatibilidade com renderer
      enemy = '../../Assets/Boss/ave/terra/armored_griffin/idle.gif';
      enemyName = 'Armored Griffin';
      enemyElement = 'terra';
      logger.info('Boss final gerado: Armored Griffin (terra)');
    } else {
      enemy = await getRandomEnemyIdle(pet ? pet.statusImage : null);
      enemyName = enemy ? require('path').basename(require('path').dirname(enemy)) : '';
      enemyElement = extractElementFromPath(enemy);
    }

    const sceneSource = data.source || 'journey'; // Captura origem ('lair' ou 'journey')
    ...
    win.webContents.on('did-finish-load', () => {
      win.webContents.send('scene-data', {
        ...
        source: sceneSource, // Passa a origem para o renderer
        boss: data && data.boss === true,
        // Preset de dificuldade "Hard Justo" para o boss final
        difficultyPreset: (data && data.boss === true)
          ? {
              name: 'hard',
              multipliers: {
                hp: 1.8,
    ...
    logger.info(`Journey scene created with enemy: ${enemyName} (${enemyElement}), source: ${sceneSource}`);
  });
  ...
  ipcMain.on('reward-pet', async (event, reward) => {
    ...
    if (reward.experience) {
      ...
      while (pet.experience >= requiredXp && pet.level < 100) {
        ...
        leveledUp = true;
        logger.info(
          `Pet ${pet.name} leveled up to ${pet.level}! Attributes gained: ${JSON.stringify(attributesGained)}`
        );
      }
