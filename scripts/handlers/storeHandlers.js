...
function registerStoreHandlers(deps = {}) {
  ...
  const giftCodes = {
    KADIR5: {
      ...
      amount: 5,
      name: '5 Kadir Points',
      icon: 'Assets/Icons/dna-kadir.png',
    },
    KADIR2025: {
      type: 'kadirPoints',
      amount: 50,
      name: '50 Kadir Points',
      icon: 'Assets/Icons/dna-kadir.png',
    },
  ...
  ipcMain.handle('use-item', async (event, itemKey) => {
    ...
    switch (itemKey) {
      case 'healthPotion':
        pet.currentHealth = Math.min(pet.currentHealth + 20, pet.maxHealth);
        break;
      case 'meat':
        // Carne: +30 fome (saciedade), +10 felicidade, cura fixa de 15 vida
        pet.hunger = Math.min((pet.hunger || 0) + 30, 100);
        pet.happiness = Math.min((pet.happiness || 0) + 10, 100);
        pet.currentHealth = Math.min(pet.currentHealth + 15, pet.maxHealth);
        break;
      case 'staminaPotion':
        pet.energy = Math.min((pet.energy || 0) + 20, 100);
        break;
      case 'chocolate':
  ...
  ipcMain.on('unequip-item', async () => {
    ...
    pet.items = items;
    try {
      await petManager.updatePet(pet.petId, { equippedItem: null });
    } catch (err) {
  ...
  ipcMain.on('redeem-gift-code', (event, code) => {
    const giftWindow =
      deps.windowManager && deps.windowManager.getGiftWindow && deps.windowManager.getGiftWindow();
    ...
    try {
      if (gift.type === 'kadirPoints') {
        const pet = getCurrentPet();
        if (pet) {
          pet.kadirPoints = (pet.kadirPoints || 0) + gift.amount;
          petManager.updatePet(pet.petId, { kadirPoints: pet.kadirPoints }).catch(() => {});
          broadcastPet(pet);
        }
        ...
        gift.items.forEach((it) => {
          items[it.item] = (items[it.item] || 0) + it.qty;
        });
        setItems(items);
        const pet = getCurrentPet();
        if (pet) {
  ...
  ipcMain.on('set-mute-state', (e, isMuted) => {
