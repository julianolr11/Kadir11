...
function openEquipModal() {
  ...
  Object.keys(pet.items).forEach((id) => {
    ...
    // Adiciona prefixo relativo se o caminho não começar com ../../
    const iconPath = info.icon.startsWith('../../') ? info.icon : `../../${info.icon}`;
    img.src = iconPath;
    img.alt = info.name;
...
function updateStatus() {
  ...
  const elementImageSrc =
    elementImages[pet.element.toLowerCase()] || '../../Assets/Elements/default.png';
  ...
  if (statusEquipImg) {
    ...
    if (info && info.icon) {
      const iconPath = info.icon.startsWith('../../') ? info.icon : `../../${info.icon}`;
      statusEquipImg.style.backgroundImage = `url(${iconPath})`;
    } else {
