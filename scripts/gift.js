    ...
    function loadHistory() {
        window.electronAPI.invoke('get-gift-history').then(history => {
            if (!history || history.length === 0) {
                historyList.innerHTML = '<p style="text-align:center;color:#aaa;padding:20px;">Nenhum presente resgatado ainda</p>';
                return;
            }
            ...
            history.forEach((item, index) => {
                ...
                date.textContent = dateObj.toLocaleDateString('pt-BR') + ' ' + dateObj.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' });
                ...
                header.addEventListener('click', () => {
                    ...
                    document.querySelectorAll('.history-container').forEach(c => c.classList.remove('open'));
