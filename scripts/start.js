...
if (!window.electronAPI) {
  console.error(
    'Erro crítico: window.electronAPI não está disponível. Verifique se o preload.js foi carregado corretamente.'
  );
  alert(
    'Erro: Não foi possível carregar a API do Electron. Verifique o console para mais detalhes.'
  );
}
...
document.addEventListener('keydown', (e) => {
  // Adicionar tecla ao buffer
  cheatBuffer += e.key.toUpperCase();
  
  // Limpar buffer após 2 segundos de inatividade
  ...
  if (cheatBuffer.includes(cheatCode)) {
    ...
    // Enviar evento para abrir tela de dev
    if (window.electronAPI) {
