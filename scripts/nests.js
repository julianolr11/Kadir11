...
function showHatchAnimation(newPet) {
  if (!hatchOverlay || !hatchVideo || !hatchVideoGif || !hatchName || !hatchGif || !hatchInput)
    return;
  hatchedPet = newPet;
  hatchGif.src = newPet.statusImage
    ? `../../Assets/Mons/${newPet.statusImage}`
    : newPet.image
      ? `../../Assets/Mons/${newPet.image}`
      : '../../Assets/Mons/eggsy.png';
  hatchInput.value = '';
...
function drawNests(count) {
  ...
  // Se não houver ninhos, mostrar link para comprar
  if (count === 0) {
    ...
    message.textContent = 'Você não possui ninhos';
    ...
    buyButton.className = 'button small-button';
    buyButton.textContent = 'Comprar Ninho';
    buyButton.style.cursor = 'pointer';
    buyButton.addEventListener('click', () => {
      window.electronAPI.send('store-pet');
    });

    buyLink.appendChild(message);
    buyLink.appendChild(buyButton);
    nestsContainer.appendChild(buyLink);
    return;
  }
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (egg) {
      ...
      baseImg.src = hasEggInInventory()
        ? '../../Assets/tileset/nest-plus.png'
...
function loadNests() {
  ...
  Promise.all([window.electronAPI.getNestCount(), window.electronAPI.getNestsData()]).then(
    ([count, data]) => {
