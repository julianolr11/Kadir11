...
const electronStub = {
  ipcMain: {
    on: (ch, fn) => {
      ipcStub.handlers[ch] = fn;
    },
    handle: (ch, fn) => {
      ipcStub.asyncHandlers[ch] = fn;
    },
  },
  BrowserWindow: {
...
function resetIpc() {
  ...
  ipcStub = {
    ...
    invoke: async (ch, ...args) =>
      ipcStub.asyncHandlers[ch] ? ipcStub.asyncHandlers[ch]({}, ...args) : undefined,
    getAllWindows: () => [
      { webContents: { send: (ev, data) => windowsSent.push({ ev, data }) } },
      { webContents: { send: (ev, data) => windowsSent.push({ ev, data }) } },
    ],
  };
}
...
describe('storeHandlers', () => {
    ...
    const deps = {
      ...
      petManager: {
        updatePet: async (id, data) => {
          calls.updatePet.push({ id, data });
          Object.assign(pet, data);
        },
      },
      store: {
        get: (k, d) => (storeData[k] !== undefined ? storeData[k] : d),
        set: (k, v) => (storeData[k] = v),
      },
      windowManager: {
        getGiftWindow: () => ({
          webContents: { send: (ev, d) => calls.giftWindow.push({ ev, d }) },
        }),
      },
    };
  ...
  it('usa meat aplica múltiplos efeitos', async () => {
    items.meat = 1;
    const prevHealth = pet.currentHealth;
  ...
  it('giftHistory mantém limite de 20 entradas', () => {
    ...
    resetIpc();
    const preHistory = Array.from({ length: 20 }, (_, i) => ({
      code: 'X' + i,
      name: 'n',
      icon: 'i',
  ...
  it('captura exceção no bloco try de redeem-gift-code (catch linha 212-213)', () => {
    ...
    const deps = {
      ...
      getCoins: () => 100,
      setCoins: () => {
        throw new Error('setCoins crash');
      },
      getItems: () => ({}),
      setItems: () => {},
      getNestPrice: () => 50,
    ...
    ipcStub.emit('redeem-gift-code', 'WELCOME');
    const errors = calls.giftWindow.filter(
      (e) => e.ev === 'gift-error' && /Erro ao processar/i.test(e.d)
    );
