...
describe('generateSpecie extra branches', () => {
  it('falls back to all species when no neutrals', () => {
    ...
    const specie = mod.generateSpecie({ attack:1, defense:1, speed:1, magic:1, life:1 }, 'terra');
  ...
  it('emergency fallback when specieData empty', () => {
    ...
    assert.ok(['Draconídeo','Reptilóide','Ave','Fera','Monstro','Criatura Mística'].includes(specie));
  });
});
