...
function makeIpc(){
  const h={};
  return { on:(c,f)=>h[c]=f, emit:(c,...a)=> h[c] ? h[c]({},...a) : undefined, _h:h };
}
...
describe('nestHandlers hatch-egg without webContents window branch', ()=>{
  it('skips loading events yet hatches successfully', async ()=>{
    ...
    const opts = {
      ipcMain: ipc,
      getNestsData: () => nests,
      setNestsData: (d) => { nests.splice(0, nests.length, ...d); },
      getHatchWindow: () => ({ /* missing webContents simulates branch */ }),
      generatePetFromEgg: () => ({ id: 'temp', specie: 'Pidgly', race: 'Pidgly', element: 'terra' }),
      petManager: {
        createPet: async (pet) => { createdPet = { ...pet, petId:'000099' }; return createdPet; }
      },
      broadcastPenUpdate: () => {},
