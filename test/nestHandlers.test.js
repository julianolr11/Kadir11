...
function mockIpc() {
  ...
  return {
    ...
    emit: async (channel, ...args) => {
      if (handlers[channel]) return handlers[channel]({ reply() {} }, ...args);
    },
    _handlers: handlers,
  };
}
...
describe('nestHandlers', () => {
  it('place-egg-in-nest should consume egg and add nest', async () => {
    ...
    assert.ok(
      BW._sends.some((s) => s.ch === 'nests-data-updated'),
      'broadcast nests update'
    );
  });
  ...
  it('hatch-egg should remove nest and create pet', async () => {
    ...
    assert.ok(
      BW._sends.some((s) => s.ch === 'pet-created'),
      'broadcast pet-created'
    );
  });
});
