...
describe('create-pet selectOption branches', () => {
  ...
  it('increments all stats when selecting option (early branch)', async () => {
    ...
    global.fetch = async () => ({ json: async () => ([
      { text:'Q1', options:[{ text:'A', points:{ attack:1, defense:2, speed:3, magic:4, life:5 }}] },
      { text:'Q2', options:[{ text:'A', points:{ attack:1, defense:1, speed:1, magic:1, life:1 }}] },
      { text:'Q3', options:[{ text:'A', points:{ attack:0, defense:0, speed:0, magic:0, life:0 }}] },
      { text:'Q4', options:[{ text:'A', points:{ attack:0, defense:0, speed:0, magic:0, life:0 }}] },
      { text:'Q5', options:[{ text:'A', points:{ attack:0, defense:0, speed:0, magic:0, life:0 }}] }
    ]) });
  ...
  it('triggers showElementSelection when last question answered (else branch)', async () => {
    const mod = require('../scripts/create-pet.js');
    global.fetch = async () => ({ json: async () => ([
      { text:'Q1', options:[{ text:'A', points:{ attack:0, defense:0, speed:0, magic:0, life:0 }}] },
      { text:'Q2', options:[{ text:'A', points:{ attack:0, defense:0, speed:0, magic:0, life:0 }}] },
      { text:'Q3', options:[{ text:'A', points:{ attack:0, defense:0, speed:0, magic:0, life:0 }}] },
      { text:'Q4', options:[{ text:'A', points:{ attack:0, defense:0, speed:0, magic:0, life:0 }}] },
      { text:'Q5', options:[{ text:'A', points:{ attack:0, defense:0, speed:0, magic:0, life:0 }}] }
    ]) });
    ...
    for (let i=0;i<4;i++){ mod.selectOption({ attack:0, defense:0, speed:0, magic:0, life:0 }); }
